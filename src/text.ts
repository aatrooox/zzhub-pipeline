...
// ── Paragraph counting (for image-plan) ───────────────────────────

/**
 * Count content paragraphs in clean body text.
 * Rules from image-plan SKILL.md:
 * - Separated by consecutive blank lines
 * - Pure blank lines and pure heading lines (# with no other text in block) don't count
 * - Word count is pure text (strip markdown)
 */
export function countParagraphs(cleanBody: string): {
  paragraphs: number;
  charCount: number;
} {
  ...
  // Filter: pure heading blocks (all lines start with #) don't count
  let paragraphs = 0;
  for (const block of blocks) {
    const hasNonHeading = block.some((line) => !/^\s*#{1,6}\s/.test(line));
    if (hasNonHeading) {
      paragraphs++;
    }
  }
