...
describe("countParagraphs", () => {
  test("counts basic paragraphs", () => {
    const input = "First paragraph.\n\nSecond paragraph.\n\nThird.";
    const result = countParagraphs(input);
    expect(result.paragraphs).toBe(3);
  });

  test("heading-only blocks don't count", () => {
    const input = "## Title\n\nActual paragraph.\n\n## Another Title\n\nAnother paragraph.";
    const result = countParagraphs(input);
    expect(result.paragraphs).toBe(2);
  });

  test("counts char count without whitespace", () => {
    const input = "Hello World";
    const result = countParagraphs(input);
    expect(result.charCount).toBe(10); // "HelloWorld" without space
  });
  ...
  test("mixed heading and content blocks", () => {
    const input = "## Heading\n\nParagraph 1\nMore text\n\n## Heading 2\n\nParagraph 2";
    const result = countParagraphs(input);
    expect(result.paragraphs).toBe(2);
  });
...
describe("generateSlug", () => {
  ...
  test("handles mixed CJK and English", () => {
    const slug = generateSlug("OpenClaw 管理工具介绍");
    expect(slug).toBe("openclaw");
  });
