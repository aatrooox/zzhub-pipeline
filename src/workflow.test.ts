...
describe("prepare", () => {
  ...
  test("preserves canonical route fields during revision-style prepare reruns", async () => {
    ...
    state.route.extras = [];
    state.route.account = "ancientone";
    state.route.content_profile = "jinx";
    await writeState(statePath, state);
    ...
    expect(updated.route.content_profile).toBe("jinx");
  });
...
describe("render", () => {
  ...
  test("passes poster render args as plain strings without JSON quoting", async () => {
    ...
    state.route.account_visual_params = {
      ...
      bg: "#e6f5ef",
      highlight: "#22a854",
      fallback_icon: "assets/icons/zzclub-logo-black.jpg",
    };
...
describe("config", () => {
  ...
  test("resolves workspace paths from defaults without hardcoded absolute roots", () => {
    ...
    expect(paths.blogRoot).toBe("/tmp/workspace/blog.zzao.club");
...
describe("blog sync", () => {
  test("sync-blog copies canonical markdown to the blog repo without joining workflow routing", async () => {
    ...
    const statePath = join(assetPath, "workflow-state.json");
    const postPath = join(assetPath, "post.md");
    const blogRoot = join(workspace, "blog.zzao.club");
...
// ── inferNewspicRenderSpec ──────────────────────────────────────────────────
import { inferNewspicRenderSpec } from "./workflow-materials";

describe("inferNewspicRenderSpec", () => {
  function makeNewspicState(overrides: Partial<{ newspic_render: unknown }> = {}) {
    const state = defaultState();
    state.route.primary = "wechat-newspic";
    state.intent.newspic_render = overrides.newspic_render !== undefined
      ? (overrides.newspic_render as ReturnType<typeof defaultState>["intent"]["newspic_render"])
      : null;
    return state;
  }

  test("sets single mode for short body (< 3 paragraphs and ≤ 150 chars)", () => {
    const state = makeNewspicState();
    inferNewspicRenderSpec("一段简短的内容", state);
    expect(state.intent.newspic_render).not.toBeNull();
    expect(state.intent.newspic_render?.pagination_mode).toBe("single");
  });

  test("sets multi mode with estimated min_pages for long body", () => {
    const state = makeNewspicState();
    // ~600 chars → ceil(600/300) = 2 pages
    const longBody = "正文段落一。".repeat(10) + "\n\n" + "正文段落二。".repeat(10) + "\n\n" + "正文段落三。".repeat(10);
    inferNewspicRenderSpec(longBody, state);
    expect(state.intent.newspic_render?.pagination_mode).toBe("multi");
    expect(state.intent.newspic_render?.min_pages).toBeGreaterThanOrEqual(1);
  });

  test("does not overwrite an already-set newspic_render spec", () => {
    const state = makeNewspicState();
    state.intent.newspic_render = {
    ...
    const longBody = "段落一内容。".repeat(30) + "\n\n" + "段落二内容。".repeat(30) + "\n\n" + "段落三内容。".repeat(30);
    inferNewspicRenderSpec(longBody, state);
    // Must remain unchanged
    expect(state.intent.newspic_render.pagination_mode).toBe("single");
  });

  test("does nothing when route is not newspic", () => {
    const state = defaultState();
    state.route.primary = "wechat-article";
    inferNewspicRenderSpec("任意内容", state);
    expect(state.intent.newspic_render).toBeNull();
  });

  test("min_pages is capped at 8 for extremely long bodies", () => {
    const state = makeNewspicState();
    // ~10000 chars → would exceed 8 without cap
    const veryLongBody = Array.from({ length: 20 }, (_, i) => `段落${i + 1}：${"文字内容".repeat(30)}`).join("\n\n");
    inferNewspicRenderSpec(veryLongBody, state);
    expect(state.intent.newspic_render?.pagination_mode).toBe("multi");
    expect(state.intent.newspic_render?.min_pages).toBeLessThanOrEqual(8);
  });

  test("reconcileStateArtifacts writes inferred spec to state file via attach-body", async () => {
    ...
    expect(finalState.intent.newspic_render).not.toBeNull();
    expect(finalState.intent.newspic_render?.pagination_mode).toBe("multi");
    expect(finalState.intent.newspic_render?.min_pages).toBeGreaterThanOrEqual(1);
  });
});
