...
function detectIcon(template: string, text: string, line1: string, line2: string, line3: string, fallbackIcon: string): string {
  if (template === "wechat-cover-split") {
    return join(ICONS_DIR, "zzclub-logo-black.jpg");
  }

  const combined = `${text} ${line1} ${line2} ${line3}`.toLowerCase();
  if (combined.includes("openclaw") || combined.includes("skill")) {
    return join(ICONS_DIR, "openclaw-logo.svg");
  }
...
export function runRenderCardCli(argv: string[]): void {
  ...
  const html = renderTemplate(templateHtml, {
    ...
    "{{AVATAR_PATH}}": join(ICONS_DIR, "avatar_jinx_cartoon.jpg"),
